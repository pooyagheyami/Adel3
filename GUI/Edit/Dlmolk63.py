...
class MyPanel2 ( wx.Panel ):
	...
	def __init__( self, parent ):
		wx.Panel.__init__ ( self, parent, id = wx.ID_ANY, pos = wx.DefaultPosition, size = wx.Size( 680,300 ), style = wx.TAB_TRAVERSAL )

		self.SetFont( wx.Font( 12, 74, 90, 92, False, FONT_TYPE ) )
		...
		self.titr = wx.StaticText( self, wx.ID_ANY, u"حذف املاک", wx.DefaultPosition, wx.DefaultSize, wx.ALIGN_CENTRE )
		self.titr.Wrap( -1 )
		self.titr.SetFont( wx.Font( 19, 70, 90, 92, False, wx.EmptyString ) )
		
		Vsz1.Add( self.titr, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL|wx.EXPAND, 5 )
		
		Hsz0 = wx.BoxSizer( wx.HORIZONTAL )
		
		
		Hsz0.AddSpacer( ( 0, 0), 1, wx.EXPAND, 5 )
		
		chos2Choices = [ u"آپارتمان", u"ویلائی", u"مغازه", u"زمین" ]
		self.chos2 = wx.Choice( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, chos2Choices, 0 )
		self.chos2.SetSelection( 0 )
		Hsz0.Add( self.chos2, 0, wx.ALL, 5 )
		
		chos1Choices = [ u"فروش", u"رهن", u"رهن اجاره" ]
		self.chos1 = wx.Choice( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, chos1Choices, 0 )
		self.chos1.SetSelection( 0 )
		Hsz0.Add( self.chos1, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5 )
		
		self.txt = wx.StaticText( self, wx.ID_ANY, u"نوع درخواست", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.txt.Wrap( -1 )
		Hsz0.Add( self.txt, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5 )
		
		
		Vsz1.Add( Hsz0, 0, wx.EXPAND, 5 )
		
		Hsz1 = wx.BoxSizer( wx.HORIZONTAL )
		
		self.DVLC1 = wx.dataview.DataViewListCtrl( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, 0 )
		self.Col1 = self.DVLC1.AppendTextColumn( u"مبلغ/اجاره" )
		self.Col2 = self.DVLC1.AppendTextColumn( u"پيش/متري" )
		self.Col3 = self.DVLC1.AppendTextColumn( u"مفيد" )
		self.Col4 = self.DVLC1.AppendTextColumn( u"متراژ" )
		self.Col5 = self.DVLC1.AppendTextColumn( u"آدرس" ,align=wx.ALIGN_RIGHT)
		self.Col6 = self.DVLC1.AppendTextColumn( u"منطقه" ,align=wx.ALIGN_RIGHT)
		self.Col7 = self.DVLC1.AppendTextColumn( u"نام ملک" ,align=wx.ALIGN_RIGHT)
		self.Col8 = self.DVLC1.AppendTextColumn( u"نوع ملک" ,align=wx.ALIGN_RIGHT)
		self.Col9 = self.DVLC1.AppendTextColumn( u"کد ملک" )
		...
		self.sql1 = " and Molks.Res = %s "%'1'
		self.sql2 = " and Mltype.Mtype = %s "%'0'
		data = self.iMolk.gMolks2(self.sql1+self.sql2)
		
		#for itemvalues in data:
                #        self.DVLC1.AppendItem(itemvalues)

                for d in data:
                        d = (self.inum.Digigrop(d[0],','),self.inum.Digigrop(d[1],','),
                             self.inum.e2f(d[2]),self.inum.e2f(d[3]),d[4],d[5],d[6],d[7],self.inum.e2f(d[8]))
		...
		# Connect Events
		self.chos2.Bind( wx.EVT_CHOICE, self.typchs )
		self.chos1.Bind( wx.EVT_CHOICE, self.reschs )
	...
	def typchs( self, event ):
		ityp = event.GetEventObject().GetSelection()
		self.sql2 = " and Mltype.Mtype = %s "%unicode(int(ityp))
		self.DVLC1.DeleteAllItems()
		self.DVLC1.Refresh()   
		data = self.iMolk.gMolks2(self.sql2+self.sql1)
		#for itemvalues in data:
                #    self.DVLC1.AppendItem(itemvalues)
                for d in data:
                        d = (self.inum.Digigrop(d[0],','),self.inum.Digigrop(d[1],','),
                ...
                self.DVLC1.Refresh()      
	def reschs( self, event ):
		ires = event.GetEventObject().GetSelection()
		#print ires
		self.sql1 = " and Molks.Res = %s "%unicode(int(ires)+1)
		self.DVLC1.DeleteAllItems()
		self.DVLC1.Refresh()   
		data = self.iMolk.gMolks2(self.sql1+self.sql2)
		#for itemvalues in data:
	...
	def selcit( self, event ):
                ...
                mtyp = self.chos2.GetStringSelection()
                mcod = self.inum.f2e( self.DVLC1.GetValue(r,8) )
                mres = self.chos1.GetSelection()
                #chtyp = self.chs2.GetStringSelection()
                sres  = self.chos1.GetStringSelection()
                #print mtyp
                #print sres
		...
		self.panel = MyPanel1(self.b,titr,txts,cods)
		#self.panel.SetLayoutDirection(2)
		self.b.SetSize((485,435))
		self.b.SetTitle(u" حذف "+ttr)
	...
	def cancl( self, event ):
        ...
        def getdata( self ):
                ...
                d = []
                for i in range(7):
                    d.append(self.DVLC1.GetValue(a,i))
                return d    
...
class MyPanel1 ( wx.Panel ):
	...
	def __init__( self, parent, titr,txts,cods ):
		...
		self.btn1 = wx.Button( self, wx.ID_ANY, u"حذف /آرشيو شود", wx.DefaultPosition, wx.DefaultSize, 0 )
	...
	def savit( self, event ):
                ...
                MessageBox(u'اطلاعات به آرشيو منتقل شد',u'اعلان',OK | ICON_INFORMATION)
                #self.filnul()
		#event.Skip()
                #print self.GetGrandParent()
                q2 = self.GetGrandParent()
                q = self.GetParent()
                q2.Close()
                q.Close()
	...
	def Gnrdat( self ):
                ...
                if len(D1[0]) > 1:
                    ...
                    DD1 = self.iMolk.gAccod( D1[0][0][0] )[0]
                    GD1 = [DD1[0],DD1[1],DD1[2],DD1[3],self.iRes]
                    
                    
                if  self.iMolk.gAlstMlk() == []:
                    self.iMcod = u'1001'
                else:
                    self.iMcod = unicode(int(self.iMolk.gAlstMlk()[-1][0])+1)
                 
                if self.iMolk.gAMtgCk(D1[1][0]) == [] :
                    iMgcod = unicode(len(self.iMolk.gAMntgh()))
                    self.jMolk.InAmntg(" Area , Aname ",[iMgcod,D1[1][0]])
                    #self.Massage(u"منطقه جديد به ثبت رسيد")
                else:
                    iMgcod = self.iMolk.gAMtgCk(D1[1][0])[0][0]
                                                
                Adrs = hex(int(self.iMcod))[2:]+'-'+iMgcod+str(self.iMtyp)+self.iRes
                MSpc = hex(int(self.iMcod))[2:]+'-'+str(self.iMtyp)+self.iRes+iMgcod
                des  = hex(int(self.iMcod))[2:]+'-'+self.iRes+iMgcod+str(self.iMtyp)
                GD2 = [D1[2][0],self.iMcod,self.iRes,self.iMtyp,iMgcod,
        ...
        def DelAcc(self,Data):
                self.jMolk.InAAcc("Acc , name, phone ,Spc ,Res ",Data)
                #self.jMolk.DelAcc(" Acc = '%s' "%self.macc)
        def DelMlk(self,Data):
                self.jMolk.InAMolk(" Date, Mcode , Res , Mtype , Area \
                                  ,Mname , MSpc , Addrss , M1 , M2 , M3 , M4 \
                                  , Acc , Amunt1 , Amunt2 , des ",Data)
                self.jMolk.DelMolk(" Mcode = '%s' "%self.mcod)
        def DelMpc(self,Data):
                self.jMolk.InAmspc(" MSpc , Spcfy ",Data)
                self.jMolk.DelMsp(" MSpc = '%s' "%self.mspc)
        def Deldes(self,Data):
                self.jMolk.InADes(" des , dscrpt ",Data)
                self.jMolk.DelDes(" des = '%s' "%self.mdes)

        def setdata( self , mcod ):
                data = self.iMolk.ShwMlks2(mcod)[0]
                #print data
                Data1 = [data[0],data[1],data[2],data[3],data[4],data[5],unicode(data[6]),unicode(data[7]),unicode(data[9]),unicode(data[8])]
                #Data2 = [data[9]]
                Data3 = [unicode(data[11]),unicode(data[12]),data[13]]
                self.P1.Setfld(Data1)
                Data2 = self.GnSpc( data[10])
                #print Data2
                                    
                if self.iMtyp == 2:
                    self.pnl2.Setchs(Data2)
                elif self.iMtyp == 3:
                    self.pnl3.Setchs(Data2)
                elif self.iMtyp == 1:
                    self.pnl1.Setchs(Data2)    
                else:
                    self.pnl1.Setchs(Data2)
                self.P4.Setfld(Data3)
                cods = self.iMolk.gMCods(mcod)
                self.macc = cods[0][0]
                self.mspc = cods[0][1]
                self.mdes = cods[0][2]
                self.Refresh()
...
class MyPanel4 ( wx.Panel ):
	...
	def __init__( self, parent , txts ):
		...
		Hsz5.Add( self.txt11, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5 )
		...
		Hsz5.Add( self.txt10, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5 )
